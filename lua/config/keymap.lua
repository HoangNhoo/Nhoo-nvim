...
wk.add({
	...
	{ "c", group = "[c]ode / [c]ell / [c]hunk" },
	{
		{ "cn", new_terminal_shell, desc = "[n]ew terminal with shell" },
		{
			"cr",
			function()
		...
		{ "cp", new_terminal_python, desc = "new [p]ython terminal" },
		{ "ci", new_terminal_ipython, desc = "new [i]python terminal" },
		{ "cj", new_terminal_julia, desc = "new [j]ulia terminal" },
	},
	{ "e", group = "[e]dit" },
	{ "d", group = "[d]ebug" },
	{
	...
	{ "f", group = "[f]ind (telescope)" },
	{
		{ "ff", "<cmd>Telescope find_files<cr>", desc = "[f]iles" },
		{ "fh", "<cmd>Telescope help_tags<cr>", desc = "[h]elp" },
		{ "fk", "<cmd>Telescope keymaps<cr>", desc = "[k]eymaps" },
		{ "fg", "<cmd>Telescope live_grep<cr>", desc = "[g]rep" },
		{ "fb", "<cmd>Telescope current_buffer_fuzzy_find<cr>", desc = "[b]uffer fuzzy find" },
		{ "fm", "<cmd>Telescope marks<cr>", desc = "[m]arks" },
		{ "fM", "<cmd>Telescope man_pages<cr>", desc = "[M]an pages" },
		{ "fc", "<cmd>Telescope git_commits<cr>", desc = "git [c]ommits" },
		{ "f<space>", "<cmd>Telescope buffers<cr>", desc = "[ ] buffers" },
		{ "fd", "<cmd>Telescope buffers<cr>", desc = "[d] buffers" },
		{ "fq", "<cmd>Telescope quickfix<cr>", desc = "[q]uickfix" },
		{ "fl", "<cmd>Telescope loclist<cr>", desc = "[l]oclist" },
		{ "fj", "<cmd>Telescope jumplist<cr>", desc = "[j]umplist" },
	},
	{ "g", group = "[g]it" },
	{
		{ "gc", ":GitConflictRefresh<cr>", desc = "[c]onflict" },
		{ "gs", ":Gitsigns<cr>", desc = "git [s]igns" },
		{
			"gwc",
			":lua require('telescope').extensions.git_worktree.create_git_worktree()<cr>",
			desc = "worktree create",
		},
		{ "gws", ":lua require('telescope').extensions.git_worktree.git_worktrees()<cr>", desc = "worktree switch" },
		{ "gd", group = "[d]iff" },
		{
			{ "gdo", ":DiffviewOpen<cr>", desc = "[o]pen" },
			{ "gdc", ":DiffviewClose<cr>", desc = "[c]lose" },
		},
		{ "gb", group = "[b]lame" },
		{
			{ "gbb", ":GitBlameToggle<cr>", desc = "[b]lame toggle virtual text" },
			{ "gbo", ":GitBlameOpenCommitURL<cr>", desc = "[o]pen" },
			{ "gbc", ":GitBlameCopyCommitURL<cr>", desc = "[c]opy" },
		},
	},
	{ "h", group = "[h]elp / [h]ide / debug" },
	{
		{ "hc", group = "[c]onceal" },

		{ "hh", ":set conceallevel=1<cr>", desc = "[h]ide/conceal" },
		{ "hs", ":set conceallevel=0<cr>", desc = "[s]how/unconceal" },
	},
	{ "t", group = "[t]reesitter" },
	{
		{ "tt", vim.treesitter.inspect_tree, desc = "show [t]ree" },
	},
	{ "i", group = "[i]mage" },
	{ "l", group = "[l]anguage/lsp" },
	{
		{ "lr", vim.lsp.buf.references, desc = "[r]eferences" },
		{ "R", desc = "[R]ename" },
		{ "lD", vim.lsp.buf.type_definition, desc = "type [D]efinition" },
		{ "la", vim.lsp.buf.code_action, desc = "code [a]ction" },
		{ "le", vim.diagnostic.open_float, desc = "diagnostics (show hover [e]rror)" },
		{ "ld", group = "[d]iagnostics" },
		{
			{
				"ldd",
				function()
			...
			{ "lde", vim.diagnostic.enable, desc = "[e]nable" },
		},
		{ "lg", ":Neogen<cr>", desc = "neo[g]en docstring" },
	},
	{ "o", group = "[o]tter & c[o]de" },
	{
		{ "oa", require("otter").activate, desc = "otter [a]ctivate" },
		{ "od", require("otter").deactivate, desc = "otter [d]eactivate" },
		{ "oc", "O# %%<cr>", desc = "magic [c]omment code chunk # %%" },
		{ "or", insert_r_chunk, desc = "[r] code chunk" },
		{ "op", insert_py_chunk, desc = "[p]ython code chunk" },
		{ "oj", insert_julia_chunk, desc = "[j]ulia code chunk" },
		{ "ob", insert_bash_chunk, desc = "[b]ash code chunk" },
		{ "oo", insert_ojs_chunk, desc = "[o]bservable js code chunk" },
		{ "ol", insert_lua_chunk, desc = "[l]lua code chunk" },
	},
	{ "q", group = "[q]uarto" },
	{
	...
	{ "r", group = "[r] R specific tools" },
	{
		{ "rt", show_r_table, desc = "show [t]able" },
	},
	{ "v", group = "[v]im" },
	{
		{ "vt", toggle_light_dark_theme, desc = "[t]oggle light/dark theme" },
		{ "vc", ":Telescope colorscheme<cr>", desc = "[c]olortheme" },
		{ "vl", ":Lazy<cr>", desc = "[l]azy package manager" },
		{ "vm", ":Mason<cr>", desc = "[m]ason software installer" },
		{ "vs", ":e $MYVIMRC | :cd %:p:h | split . | wincmd k<cr>", desc = "[s]ettings, edit vimrc" },
		{ "vh", ':execute "h " . expand("<cword>")<cr>', desc = "vim [h]elp for current word" },
	},
	{ "x", group = "e[x]ecute" },
	{
